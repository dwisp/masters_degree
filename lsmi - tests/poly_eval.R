...
test.vec1 <- split(rnorm(45), rep(1:15, 3))
test.vec2 <- split(rnorm(45), rep(1:15, 3))

lsmi.poly(test.vec1, test.vec2)
...
nl.ex1.lsmi <- lsmi.poly(nl.ex1.x, nl.ex1.y) %>% round(3)
...
ggplot(data.frame(x = nl.ex1.x, y = nl.ex1.y), aes(x, y)) + 
  ...
  ggtitle(paste('Nonlinear dependence example 1: high-frequency sine function\n x ~ U[-1;1]; y = sin 4*pi*x + N(0, 0.25); n = 50\n',
                'Cor = ', nl.ex1.cor, '; ', 'polyLSMI = ', nl.ex1.lsmi, sep = '')) +
...
nl.ex2.lsmi <- lsmi.poly(nl.ex2.x, nl.ex2.y) %>% round(3)
...
ggplot(data.frame(x = nl.ex2.x, y = nl.ex2.y), aes(x, y)) + 
  ...
  ggtitle(paste('Nonlinear dependence example 2: parabola\n x ~ U[-1;1]; y = x^2 + N(0, 0.1); n = 50\n',
                'Cor = ', nl.ex2.cor, '; ', 'polyLSMI = ', nl.ex2.lsmi, sep = '')) +
  theme(plot.background = element_rect(fill = "darkseagreen"), plot.title = element_text(hjust = 0.5)) +
...
nl.ex3.lsmi <- lsmi.poly(nl.ex3.x, nl.ex3.y) %>% round(3)
...
ggplot(data.frame(x = nl.ex3.x, y = nl.ex3.y), aes(x, y)) + 
  ...
  ggtitle(paste('Nonlinear dependence example 3: unit circle\n x ~ U[-1;1]; y = random(-1;1)*sqrt(1 - x^2) + N(0, 0.1), n = 50\n',
                'Cor = ', nl.ex3.cor, '; ', 'polyLSMI = ', nl.ex3.lsmi, sep = '')) +
  theme(plot.background = element_rect(fill = "darkseagreen"), plot.title = element_text(hjust = 0.5)) +
  ggsave('circle_example_poly.png', width = 9, height = 9)

# lsmi.reps.nl.ex3 <- replicate(50, lsmi.poly(as.list(nl.ex3.x), as.list(nl.ex3.y)), TRUE)
...
ggplot(data = nlin.lsmi.values_par, aes(noise, value)) + 
  ...
  scale_color_hue(labels = c("polyLSMI estimate", "Cor estimate")) +
  ...
  ggtitle(str_c('polyLSMI / Cor estimates (95% CI) vs. noise strength\n x ~ U[-1;1]; Y = X^2 + N(0, sigma = noise), n = ', nlin.dots, ';')) + 
